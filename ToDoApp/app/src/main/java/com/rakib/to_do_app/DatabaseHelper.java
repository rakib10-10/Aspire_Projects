...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String KEY_AUTH_IMAGE = "profile_image"; // NEW COLUMN

    // --- CREATE STATEMENTS ---
    // ... (Keep existing CREATE statements for TASKS, PROFILE, NOTIF, REMINDERS, NOTES) ...
    ...
    private static final String CREATE_TABLE_AUTH = "CREATE TABLE " + TABLE_AUTH + "("
            ...
            + KEY_AUTH_PHONE + " TEXT,"
            + KEY_AUTH_BIO + " TEXT,"
            + KEY_AUTH_IMAGE + " TEXT" + ")"; // Added Image Column
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        // ... (Keep existing upgrade logic for versions 4-8) ...
        if (oldVersion < 10) {
            try {
                // Add image column to AUTH table if it doesn't exist
                db.execSQL("ALTER TABLE " + TABLE_AUTH + " ADD COLUMN " + KEY_AUTH_IMAGE + " TEXT");
            } catch (Exception e) {
    ...
    // ... (Keep generic getters like isDarkModeEnabled, addTask, etc. exactly as they were) ...
    ...
    public List<Task> getAllTasks() {
        ...
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery("SELECT * FROM " + TABLE_TASKS + " ORDER BY " + KEY_CREATED_AT + " DESC", null);
        if (cursor.moveToFirst()) {
            do {
                Task task = new Task();
                task.setId(cursor.getLong(cursor.getColumnIndexOrThrow(KEY_ID)));
    ...
    public UserProfile getUserProfile() {
        ...
        if (cursor.moveToFirst()) {
            ...
            profile = new UserProfile("local_user", "Rakibul Hasan Bhuiyan", "mdtara0199@gmail.com", "+880 1316787455", "Bio", "");
    ...
    public boolean registerUser(String name, String email, String password, String phone, String bio) {
        ...
        values.put(KEY_AUTH_IMAGE, ""); // Initialize with empty image
    ...
    // --- UPDATED: GET USER DETAILS (NOW FETCHES IMAGE) ---
    ...
    // --- NEW METHOD: UPDATE LOGGED IN USER INFO ---
